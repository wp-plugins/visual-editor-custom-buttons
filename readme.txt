...
= I have just updated from an earlier version and now the CSS is no longer viewable in the Visual Editor or even worse, the buttons doesn´t show up at all in the Visual Editor. Why is that? =

That is because the plugin recreates some files when updating and overwrites earlier files. It´s very annoying and will be fixed in a future update, but for now it is fortunately very easy to fix. Just open any of the buttons and resave it. Then everything should be working again. If the CSS is still not working (you can´t se any effect of the button in the Visual Editor), just clean the Browser Cache to force the Browser to reload the CSS. That should do the trick.
